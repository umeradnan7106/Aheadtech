...
// Static fallback data shown when Sanity has no content yet
const STATIC_CASES: Record<string, any> = {
  'dtf-brand-roas': {
    badge: 'Case Study · Ecommerce',
    title: '0.39x to 7.05x ROAS. 90 days.',
    excerpt: "They were losing money on every dollar spent on ads. We fixed their tracking, rebuilt their campaigns, and turned it around in three months.",
    ...
    <>
      ...
      <section style={{ background: '#080E1C', padding: '60px 32px' }}>
        <div style={{ maxWidth: '1180px', margin: '0 auto' }}>
          ...
          {cs.excerpt && (
            <p style={{ fontSize: '15px', color: '#A4B3C4', maxWidth: '600px', lineHeight: 1.65, marginBottom: '28px', fontFamily: 'var(--font-jakarta)' }}>{cs.excerpt}</p>
          )}
      ...
      <section style={{ background: '#fff' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto', padding: '60px 32px' }}>
          {cs.problem?.length > 0 && (<>
            ...
            {cs.problem.map((p: string, i: number) => (
              <p key={i} style={{ fontSize: '15px', color: '#6E8098', lineHeight: 1.8, marginBottom: '14px', fontFamily: 'var(--font-jakarta)' }}>{p}</p>
            ))}
          </>)}
